...
class Solution:
    ...
    # This works fine for small inputs but runs out of memory for larger inputs.
    # We can change the code to iterate so that the memory issue is gone but the time to run will be long.
    def findSubstring(self, s: str, words: list[str]) -> list[int]:
        perms = list(itertools.permutations(words))
        string_perms = set([''.join(p) for p in perms])
        result = []
        for string_perm in string_perms:
            result.extend([m.start() for m in re.finditer(f"(?={string_perm})", s)])
        return result
...
#print(result)  # Expected output: [0,9]
